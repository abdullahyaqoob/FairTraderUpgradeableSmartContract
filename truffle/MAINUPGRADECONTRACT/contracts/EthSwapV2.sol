...
contract EthSwapV2 {
    ...
    function adminWithdraw(address payable recipient) external payable {
        require(recipient == adminWalletAddress, "You are not Owner");
        recipient.transfer(address(this).balance);
    }

    function adminWithdrawToken(address payable recipient) external payable {
        require(recipient == adminWalletAddress, "You are not Owner");
        token.transfer(adminWalletAddress, token.balanceOf(address(this)));
    }
    ...
    function _withdrawStake(uint256 index) public returns (uint256) {
